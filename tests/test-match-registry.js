...
import { createMatch, resolveToken, getMatch, deleteMatch } from '../src/matchRegistry.js';
...
const match1 = createMatch('match-1', ['alice', 'bob']);
...
const resolvedAlice = resolveToken(tokAlice);
...
const resolvedBob = resolveToken(tokBob);
assert('resolves bob token to correct playerId', resolvedBob.playerId, 'bob');

assertNull('unknown token returns null', resolveToken('bogus-token'));
assertNull('empty token returns null', resolveToken(''));
assertNull('null token returns null', resolveToken(null));
...
const fetched = getMatch('match-1');
...
assertNull('getMatch on unknown id returns null', getMatch('nope'));
...
const match2 = createMatch('match-2', ['charlie']);
const tokCharlie = match2.tokens['charlie'];
assertNotNull('token resolves before delete', resolveToken(tokCharlie));

deleteMatch('match-2');
assertNull('getMatch returns null after delete', getMatch('match-2'));
assertNull('token no longer resolves after delete', resolveToken(tokCharlie));
...
createMatch('match-3', ['dave']);
const replaced = createMatch('match-3', ['eve']);
assert('re-registering a matchId replaces old entry', replaced.players, ['eve']);
assertNull('old player token is invalidated after replacement', resolveToken(replaced.tokens['dave']));
